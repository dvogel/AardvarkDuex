aardvark.loadObject({
  ...
  removeBoxFromBody: function () {
    ...
    if (this.borderElems != null) {
      for (var i = 0; i < 4; i++) { this.doc.body.removeChild(this.borderElems[i]); }
  ...
  clearBox: function () {
    ...
    if (this.borderElems != null) {
      for (var i = 0; i < 4; i++) { this.borderElems[i].style.display = 'none'; }
  ...
  showKeybox: function (command) {
    if (this.keyboxElem == null) { return; }
    ...
    if (y < 0) { y = 0; } else if (y > (dims.scrollY + dims.height) - 30) { y = (dims.scrollY + dims.height) - 60; }
    var x = this.mousePosX + 10;
    if (x < 0) { x = 0; } else if (x > (dims.scrollX + dims.width) - 60) { x = (dims.scrollX + dims.width) - 100; }
  ...
  findValidElement: function (elem) {
    while (elem) {
      if (this.alwaysValidElements[elem.tagName]) { return elem; } else if (this.validIfBlockElements[elem.tagName]) {
        if (this.doc.defaultView) {
          if (this.doc.defaultView.getComputedStyle
          (elem, null).getPropertyValue('display') == 'block') { return elem; }
        } else if (elem.currentStyle) {
          if (elem.currentStyle.display == 'block') { return elem; }
        }
      } else if (this.validIfNotInlineElements[elem.tagName]) {
        if (this.doc.defaultView) {
          if (this.doc.defaultView.getComputedStyle
          (elem, null).getPropertyValue('display') != 'inline') { return elem; }
        } else if (elem.currentStyle) {
          if (elem.currentStyle.display != 'inline') { return elem; }
        }
      }
  ...
  mouseMove: function (evt) {
    if (!evt) { evt = aardvark.window.event; }
  ...
  mouseOver: function (evt) {
    ...
    if (elem.isAardvark) {
      if (elem.isKeybox) { aardvark.hideKeybox(); } else if (elem.isLabel) { aardvark.clearBox(); } else { aardvark.isOnAardvarkElem = true; }
      return;
    }
  ...
  keyDown: function (evt) {
    ...
    if (command) {
      if (command.noElementNeeded) {
        if (command.func.call(aardvark) == true) { aardvark.showKeybox(command); }
      } else {
        if (aardvark.selectedElem &&
        (command.func.call(aardvark, aardvark.selectedElem) == true)) { aardvark.showKeybox(command); }
      }
    }
...
aardvark.addCommand('xpath', function (e) {
  if (window.SimplePath) { SimplePath.showPathInSnippetEditor(e); } else { alert('please load snippet editor'); }
});
