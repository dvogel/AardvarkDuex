aardvark.loadObject({
  ...
  loadCommands: function () {
    ...
    for (var i = 0; i < keyCommands.length; i++) { this.addCommandInternal(keyCommands[i]); }
  },
  ...
  addCommand: function (name, func,
    ...
    if (!suppressMessage) { this.showMessage("<p style='color: #000; margin: 3px 0 0 0;'>command \"<b>" + name + '</b>" added</p>'); }
    ...
    if (o.parentNode != null) {
      if (this.undoData.mode == 'R') {
        ...
        if (e.nodeName == 'TR' && o.nodeName != 'TR') {
          ...
          var len = e.childNodes.length; var i; var a = [];

          for (i = 0; i < len; i++) { a[i] = e.childNodes.item(i); }
        ...
        if (o.nodeName == 'TD' && e.nodeName != 'TD') { o.insertBefore(e, o.firstChild); } else if (o.nodeName == 'TR' && e.nodeName != 'TR') { o.insertBefore(e, o.firstChild.firstChild); } else { o.parentNode.insertBefore(e, o); }
        this.clearBox();
  ...
  isolateElement: function (o) {
    if (o.parentNode != null) {
      ...
      var tmpUndoData = [];
      var len = this.doc.body.childNodes.length; var i; var count = 0; var e;
      ...
      for (i = count - 1; i >= 0; i--) { this.doc.body.removeChild(tmpUndoData[i]); }
      ...
      case 1: // ELEMENT_NODE
        {
          ...
          if (!isLeaf) {
            for (var i = 0; i < node.childNodes.length; i++) { this.deWidthify(node.childNodes.item(i)); }
          }
        }
        break;
    }
      ...
      case 1: // ELEMENT_NODE
        {
          if (node.tagName != 'IMG') {
            ...
            if (!isLeaf) {
              for (var i = 0; i < node.childNodes.length; i++) { this.blackOnWhite(node.childNodes.item(i), isLink); }
            }
          }
        }
        break;
      ...
      case 1: // ELEMENT_NODE
        {
          ...
          if (isTbody) {
            for (var i = 0; i < node.childNodes.length; i++) { str += this.getOuterHtmlFormatted(node.childNodes.item(i), indent); }
          } else {
          ...
          node.attributes.item(i).nodeValue != '') {
                ...
                if (node.attributes.item(i).nodeName == 'style') {
                  ...
                  for (var j = 0; j < a.length; j++) {
                    var pair = a[j].split(':');
                    if (pair.length == 2) {
                      var s = this.trimSpaces(pair[0]); var index;
            ...
            if (isLeaf) { str += ' /&gt;\n</div>\n'; } else {
              str += '&gt;\n</div>\n';
              for (var i = 0; i < node.childNodes.length; i++) { str += this.getOuterHtmlFormatted(node.childNodes.item(i), indent + 1); }
              str += "\n<div class='vsline'>\n&lt;/<span class='tag'>" +
          node.nodeName.toLowerCase() + '</span>&gt;\n</div>\n</div>\n';
            }
          }
      ...
      case 3: // TEXT_NODE
        {
          ...
          v = this.trimSpaces(v);
          if (v != '' && v != '\n' &&
        v != '\r\n' && v.charCodeAt(0) != 160) { str += "<div class='vsindent'>" + v + '</div>'; }
        }
        break;
  ...
  domJavascript: function (node, indent) {
    var indentStr = '';
    for (var c = 0; c < indent; c++) { indentStr += '  '; }
      ...
      case 1: // ELEMENT_NODE
        {
          ...
          var children = ''; var numChildren = 0; var t; var useInnerHTML = false;
          if (!isLeaf) {
              ...
              switch (n) {
                case 'style': {
                  ...
                  for (var j = 0; j < a.length; j++) {
                    ...
                    if (pair.length == 2) {
                      var s = this.trimSpaces(pair[0]); var index;
                      while ((index = s.indexOf('-')) != -1) { s = s.substring(0, index) + s.charAt(index + 1).toUpperCase() + s.substring(index + 2); }
          ...
          if (useInnerHTML) {
            var ih = node.innerHTML; var index;
          ...
          if (numChildren) {
            if (numProps) { return str + properties + ',\n' + children + indentStr + ')'; } else { return str + '\n' + children + indentStr + ')'; }
          } else
          if (numProps) { return str + properties + ')'; } else { return str + ')'; }
        }
        break;
  ...
  undo: function () {
    ...
    switch (ud.mode) {
      case 'I': {
        ...
        for (i = count - 1; i >= 0; i--) { this.doc.body.removeChild(a[i]); }

        len = this.undoData.numElems;
        for (i = 0; i < len; i++) { this.doc.body.appendChild(this.undoData[i]); }
      ...
      case 'R': {
        if (ud.nextSibling) { ud.parent.insertBefore(ud.elem, ud.nextSibling); } else { ud.parent.appendChild(ud.elem); }
        break;
      }
      default:
        return false;
  ...
  makeGlobalFromElement: function (elem) {
    if (this.isBookmarklet) {
      ...
      // within the page
      if (this.doc.aardvarkElemNum == null) { this.doc.aardvarkElemNum = 1; } else { this.doc.aardvarkElemNum++; }
      var removeId = false;
  ...
  addChildren: function (elem, t, depth) {
    for (var i = 0; i < elem.childNodes.length; i++) {
      ...
      if (child.nodeType == 1) {
        ...
        if (child.childNodes.length != 0 && !aardvark.leafElems[child.nodeName]) { aardvark.addChildren(child, t, depth + 1); }
      }
    }
  }
