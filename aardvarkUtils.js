...
// -----------------------------------------------------
  ...
  setHandler: function (obj, eventName, code) {
    if (aardvark.doc.all) { obj.attachEvent('on' + eventName, code); } else { obj.addEventListener(eventName, code, false); }
  },
    ...
    switch (node.nodeType) {
      case 1: { // ELEMENT_NODE
        ...
        if (isLeaf) { str += ' />'; } else {
          ...
          for (var i = 0; i < node.childNodes.length; i++) { str += aardvark.getOuterHtml(node.childNodes.item(i)); }
  ...
  // test for IE (can i just use "aardvark.doc.all"?)
    ...
    if (!isIE) { style_node.appendChild(aardvark.doc.createTextNode(selector + ' {' + declaration + '}')); }
